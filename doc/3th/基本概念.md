# 基本概念

### 语法

1. 区分大小写
变量 `test` 和 `Test` 分别不同变量，变量函数名不能使用关键字。

2. 标识符
变量标识符可以用一个正则表达式去表达
`var regExp = \^(\$|[a-zA-Z]|\_){1}\w*?$\g`

![image](/img/identifier.png)

标识符采用驼峰大不小写格式：
`myCar firstSecond`

不能用关键字写标识符。

3. 注释

```html
// 单行注释
```

```html
/*
 *  多行
 * （块级)注释
 */
```

4. 严格模式

严格模式 `"use strict"` 是 ECMAScript5 引入的编译指示器（pragma）概念，是将 ECMAScript3 中的一些不确定行为规范化，如果使用不恰当会编译时报错。

5. 语句
```javascript
var sum = a + b
var diff = a - b;
```
有没分号都是有效语句，不过为了保险都是推荐带分号的语句。不过重此 JS 分了有分派和无分派。

6. 关键字和保留字

泄及编写程序语法的字，不一一列举了。

7. 变量

ECMAScript 的变量是松散类型的，定义后可以保留任何值。

```javascript
var message = "hi"
message = 100
```

定义变量时省略了 var，会变成全局变量。


### 数据类型


1. 数据类型

ECMAScript 现在存有的数据类型有：

基础类型： Undefined, Null, Boolean, Number, String

引用类型： Object, Function, Array, Symble

2. `typeof` 操作符

因为 ECMAScript 的变量是松散类型的，`typeof`是一个用来判断 JS 变量类型的方法，但是 `typeof` 有明显的缺陷，`typeof` 只能判断变量的基础类型， 引用类型全部返回 `"object"`

3. `Undefined` 类型和 `Null` 类型

- `undefined` 类型代表一个变量的值未初始化。
- `null` 类型代表一个变量为空值，并没有数据或者类型存在里面。

有时候他们是相等的, `undefined` 值派生自 `null` 值。

```javascript
console.log(null == undefined) // true
```

4. Boolean 类型

```javascript
Boolean(0) //false
Boolean(1) //true
Boolean("") //false
Boolean("222")//true
Boolean(null)// false
Boolean(undefined)//false
Boolean([])//true
Boolean([1,"2",5])//true
Boolean({})//true
Boolean({
    name: "nickname"
})//true
Boolean(NaN) //false
```

5. Number 类型


Number 大多数浏览器中支持 5e-324 这个值。如果某次计算的结果得到了一个超过JavaScript范围的值，那么这个数值将被转换成 Infinity 值。可以使用 `isFinite()` 判断。

Number类型支持，十进制数，八进制数，十六进制数:

- 十进制数: `var intNum = 55 //整数` 

- 八进制数: `var octalNum = 070 //八进制的56`

八进制的数字面值第一位必须是0

- 十六进制数： `var hexNum = 0xA //十六进制的10`

十六进制的字面值前两位必须是 0x

- 浮点数: `var floatNum = 12.5`

`var floatNum = 3.125e7 //等于 31250000 `

不要测试浮点的值，不然会容易冒出 `0.1+0.2 = 3.00000004` 的情况。


最大的保存值： `Number.MAX_VALUE`

最小的保存值： `Number.MIN_VALUE`

Number 类型转换:

```javascript
Number(false) //0
Number(true) //1
Number("") //0
Number("333") //333
Number("string") //NaN
Number("00011") //11
Number(null) //0
Number(undefined) //NaN
Number([]) //0
Number([3,'tt']) //NaN
Number({}) //NaN
Number({
    name: "nickname"
}) // NaN
```

数据转换方法： `parseInt()`、`parseFloat()`

```javascript
//parseInt
parseInt("123blue") //123 无视字符串
parseInt("") // NaN
parseInt("0xA") //10 十六进制数
parseInt("070") //56 八进制数
parseInt(12.25) //12 十进制数自动取整，不要小数点
parseInt([]) // NaN
parseInt([5,6]) //5
parseInt(["6",5]) //6
parseInt(["tt",5]) //NaN
parseInt({}) //NaN
parseInt(null) //NaN
parseInt(undefined) //NaN

//parseFloat
parseFloat("123blue") //123
parseFloat("0xA") //0 十六进制数 读取不了当整数读取
parseFloat("070") //70 八进制数 读取不了当字符串转化
parseFloat("22.56") //22.56 
parseFloat("3.125e7") //31250000
parseFloat([]) //NaN
parseFloat(["33",6]) //33 只转换数组第一个值
parseFloat(["chan",55]) //NaN
parseFloat({})//NaN
parseFloat(null) //NaN
parseFloat(undefined) //NaN
```

6. NaN 类型

NaN,（Not a Number）非数值，是一个特殊的数值，这个数值用于表示一个本来要返回的数值的操作未返回数值的情况。NaN是一个很特别的情况。

关于 isNaN 的类型判断：

```javascript
NaN == NaN //false
isNaN(NaN) //true
isNaN(10) //false
isNaN(0)  //false
isNaN(null) //false
isNaN(undefined) //true
isNaN([]) //false
isNaN([2,3]) //true
isNaN("blue") //true
isNaN("") //false
isNaN(true) //false
isNaN(false) //false
isNaN({}) //true
isNaN({
    name: "nickname"
}) // true
```

当一个变量尝试表为数值的时候，像 `isNaN()` 也一样，都会首选调用对象 `valueOf()` 方法, 转化为其本类型值，如果能转化就直接返回，如果不能再调用 `toString()` 方法。

7. string 类型

String类型用于表示16位Unicode字符组成的字符串。

字符串有些转义字面量

'\n' 换行
'\t' 制表
'\b' 空格
'\r' 回画
'\f' 进纸

某些特殊字符转义:

'\\''单引号  '\\"'双引号  "\\xunn"十六进制等

string特性：

- 字符串有length
- 字符串可以通过 + 拼接
- 任何方法都能通过toString()转换为字符串，除了 `null` 和 `undefined`
- toString() 输入参数,toString(2)二进制 toString(8)八进制 toString(10)十进制 toString(16)进制
- 可以新建String()对象来转化字符串, `null` 和 `undefined`也行 `String(null)` `String(undefined)`

8. Object 类型

Object类型，又是一组数组和对象的集合，对象可以通过 new 创建或者字面量创建，每个对象创建后都会继承 Object 属性和方法

- Constructor 对象构建函数
- hasOwnProperty 查找属性名方法
- isPrototypeOf 检查是否另一对象的原型
- propertyIsEnumerable 检查是否可枚举的属性
- toLocaleString() 返回对象字符串执行地区的字符
- toString() 返回字符串
- valueOf() 返回对象的值

值得注意的一点是： **ECMA-262定义的对象行为不适用于浏览器BOM 和 DOM 中的对象，因为它们是由宿住实现提供和定义的**

9. 操作符

ECMA-262操作符描述了一组用于操作数据值的操作符，不同之处在于，它们能够适用于很多值，数值字，字符串，甚至对象，转化值的时候还会相应的调用方法 `valueOf()` 和 `toString()` 来转化。

- 递增和递减
分前置型和后置型，
```javascript
var age = 29
// 前置型——求值之前改变
++age //30 等同于 age = age + 1 
--age //29 等同于 age = age - 1

// 后置型——求值之后执行
var age = 29
age++ //29 先给出值，再执行 age = age + 1
age-- //30 先给出值，再执行 age = age - 1
console.log(age) // 29 最后是执行了减1的值
```
在其它类型的变量递增和递减，一般会先调用 `valueOf()` 和 `toString()` 转变成基本类型再执行递增和递减比如：
```javascript
++false //报错 Uncaught ReferenceError: Invalid left-hand side expression in prefix operation
var a = false
++a // 1
```

- 一元加和减操作符

如果一元符放在变量前面，会先调用 Object 里面的 `valueOf()` 转化为基础值 ，如果不能转何为基础值
比如 NaN 再调用 `toString()` 比如：

```javascript
var s1 = "2"
-s1 // -2 
var s2 = "str"
+s2 // NaN 不能转化为数值，再toString()

```

- 位操作符

在 ECMAScript 中所有数值都以IEEE-754 **64位格式存储的**，但操作符并不直接操作64位的值，而是先将 64位的值转换成 32位的整数，然后再执行操作，最后再将结果转换回64位，操作的过程就像是存在32位的整数一样。

32位数里面，前31位用于表示整数的值，第32位用来表示数值的符号： **0表示正数，1表示负数**。

负数的存储方式是以， 二进制补码的形式保存。需要经过3个步骤比如18：

1) 求这个绝对值的二进制码（18二进制码）。
2) 求二进制反码，将0替换为1。
3) 得到二进制反码加1。

默认的情况，32位数都是带符号的，如果不带符号可以多出一位用来表示更多的数值。

- 按位非(NOT)~

波浪线(~)，执行数值返回数值的反码。

```javascript
var num1 = 25
console.log(~num) //-26 反码
```

- 按位与(AND)&

一个和号字符(&)表示将两个数值的每一位对齐,对比两个数每一位二进制码，两个数交合的值才是最后的值。

```javascript
var result = 25 & 3
console.log(result) //1
```

- 按位或（OR）
按位或者由一个竖线符号(|)表示，对比两个数每一位二进制码，两个数其中一个有值就有值。

```javascript
var result = 25 | 3
console.log(result) //27
```

- 按位异或者(XOR)
按位异或由一个插入符号(^)表示，当两个数1 和 0 的时候结果为1，否则为0

```javascript
var result = 25 ^ 3
console.log(result) //26
```

- 左移操作符(<<) 和 右移操作符 (>>)
原理是把数值的二进制数，左移多少位，或者右移多少位。

```javascript
var oldvalue = 2
console.log(oldvalue << 1) //4 向左移一位 
var oldvalue1 = 4
console.log(oldvalue >> 1) //2 向右移一位
```

- 无符号右移
无符号右移操作符是(>>>)， 这个操作会把数值的二进制 32 位都向右移动

```javascript
var oldvalue = 64 
console.log(oldvalue >>> 5) // 等二进制的10，即十进制的2
```

- 逻辑非
逻辑非操作符由一个叹号(!)表示，对 ECMAScript 中的任何值。先将操作数转换为一个布尔值，然后再对其求反。

```javascript
console.log(!false) //true
console.log(!"string") //false
console.log(!0) //true
console.log(!NaN) //true
console.log(!"") //true
console.log(!1234) //true
console.log(![]) //false
console.log(![3,4]) //false
console.log(!{}) //false
console.log(!{name: name}) //false
```

双逻辑非(!!)返回 Boolean的值，可以用来判断 `null` 和 `undefined` 的结果。

- 逻辑与(&&)

逻辑与平时经常使用到拉，在 `if` 判断语句的时候，就是两个同时为真才为真，其它为假。

- 逻辑或(||)

逻辑或操作符由两个竖线符号(||)表示，这个也是经常在逻辑判断时用到，两个数其中一个为真，结果为真，其它为假。





### 流控制语句

### 理解函数